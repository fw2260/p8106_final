...
### Model Findings
While all models performed very well in classifying income on the training and testing datasets with AUC > 80, the boosting model was the best-performing model on both training and testing datasets. For the boosting model, as shown in Figure 7, the most important predictors used were `marital status`, `education.num`, `capital.gain`,`age`, `hours.per.week` and `occupation`. 

Non-black box model such as logistical regression also shows a similar picture in terms variable importance. The coefficient estimates demonstrate relatively larger changes in the log odds of income level are associated with a change in marital status from the referenced “Divorced” to “Married” (an increase), occupation change from “MBSA” to “Service” (decrease), relationship status change from “Husband” to “Not-in-family”(decrease), and sex from “Female” to “Male” (increase). For continuous variables, one unit increase in age, capital gain and capital loss also lead to an increase of log odds.
